...
const titleLines: Array<Array<{ text: string; em?: true }>> = [
  [{ text: 'Where' }, { text: 'Technical' }, { text: 'Depth' }],
  [{ text: 'Meets' }, { text: 'Strategic' }, { text: 'Foresight.', em: true }],
];
...
export default function ExpertiseHero() {
  ...
  useLayoutEffect(() => {
    const words     = h1Ref.current?.querySelectorAll<HTMLElement>('.eh-word') ?? [];
              ...
              {titleLines.map((line, lineIdx) => (
                <span key={lineIdx} className="block">
                  {line.map((word, wIdx) => (
                    ...
                    >
                      {word.em ? (
                        <em className="eh-word not-italic text-accent inline-block">
                          {word.text}
                        </em>
                      ) : (
                        <span className="eh-word inline-block">{word.text}</span>
                      )}
                    </span>
                  ))}
                </span>
