...
const titleLines: Array<Array<{ text: string; em?: true }>> = [
  [{ text: 'Ideas' }, { text: 'Turned' }, { text: 'Into' }],
  [{ text: 'Real-World' }, { text: 'Impact.', em: true }],
];
...
export default function ProjectsHero() {
  ...
  useLayoutEffect(() => {
    const words     = h1Ref.current?.querySelectorAll<HTMLElement>('.ph-word') ?? [];
              ...
              {titleLines.map((line, lineIdx) => (
                <span key={lineIdx} className="block">
                  {line.map((word, wIdx) => (
                    ...
                    >
                      {word.em ? (
                        <em className="ph-word not-italic text-accent inline-block">{word.text}</em>
                      ) : (
                        <span className="ph-word inline-block">{word.text}</span>
                      )}
                    </span>
                  ))}
                </span>
          ...
          <div ref={mosaicRef} className="hidden lg:flex gap-3">
            <div className="flex-1 flex flex-col gap-3 -mt-24">
              <div className="mosaic-img relative rounded-2xl overflow-hidden h-[300px]">
                <Image src={IMAGES.hero} alt="" fill className="object-cover" sizes="22vw" priority />
              </div>
              <div className="mosaic-img relative rounded-2xl overflow-hidden h-[200px]">
                <Image src={IMAGES.expertise} alt="" fill className="object-cover" sizes="22vw" />
              </div>
              <div className="mosaic-img relative rounded-2xl overflow-hidden h-[180px]">
                <Image src={IMAGES.aboutCta} alt="" fill className="object-cover" sizes="22vw" />
              </div>
            </div>
            <div className="flex-1 flex flex-col gap-3 mt-16">
              <div className="mosaic-img relative rounded-2xl overflow-hidden h-[360px]">
                <Image src={IMAGES.connect} alt="" fill className="object-cover" sizes="22vw" />
              </div>
              <div className="mosaic-img relative rounded-2xl overflow-hidden h-[220px]">
                <Image src={IMAGES.expertise} alt="" fill className="object-cover object-top" sizes="22vw" />
              </div>
            </div>
          </div>
