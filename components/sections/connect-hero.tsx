...
const titleLines: Array<Array<{ text: string; em?: true }>> = [
  ...
  [{ text: 'Something' }, { text: 'Meaningful.', em: true }],
];
...
export default function ConnectHero() {
  ...
  useLayoutEffect(() => {
    const words     = h1Ref.current?.querySelectorAll<HTMLElement>('.ch-word') ?? [];
              ...
              {titleLines.map((line, lineIdx) => (
                <span key={lineIdx} className="block">
                  {line.map((word, wIdx) => (
                    ...
                    >
                      {word.em ? (
                        <em className="ch-word not-italic text-accent inline-block">{word.text}</em>
                      ) : (
                        <span className="ch-word inline-block">{word.text}</span>
                      )}
                    </span>
                  ))}
                </span>
